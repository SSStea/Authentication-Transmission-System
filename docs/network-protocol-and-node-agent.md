...
每条NodeAgent TCP连接的首个控制消息必须是 `CLIENT_HELLO`，角色为 `MANAGER` 或 `MONITOR`。阶段3实现 `PING/PONG` 和 `STATUS_REQUEST/STATUS_RESPONSE`；阶段4增加 `SENDER_AUTH_CONFIG`、`RECEIVER_AUTH_CONTEXTS` 和 `AUTH_CONFIG_ACK`；阶段6增加独立的 `TEXT_PAYLOAD_CONFIG`、开始/暂停/继续/停止命令、命令确认和轮次结果；阶段7增加 `FILE_UPLOAD_BEGIN`、`FILE_BINARY_CHUNK`、`FILE_UPLOAD_END`以及文件模式专用结果详情。二进制文件主体只允许MANAGER上传。
...
- `AuthenticationNodeRuntime`：PC节点和NodeAgent共用的认证配置、文本载荷、轮次命令、UDP入口与结果适配；
- `AuthenticationSenderRuntime`、`AuthenticationReceiverRuntime`：实现绝对时间调度、延迟披露、原生/改进验证、有界队列和可等待停止；
- `SystemTimeSynchronization`：在开始轮次前检查systemd-timesyncd或Chrony状态，未同步时拒绝启动；
...
阶段6新增的原生/改进文本端到端、暂停恢复和调度超限测试见
[文本认证传输](stage6-text-authentication.md)。

阶段7的文件上传事务、32B切片、恢复落盘和Hash边界见
[文件认证、恢复与Hash比较](stage7-file-authentication.md)。
